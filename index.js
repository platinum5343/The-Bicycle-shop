  ...
  if(req.url === '/favicon.ico') return;
    ...
    if(pathname === '/'){
        ...
        for (let index = 0; index <6; index++){
            allMainBicycles += replaceTemplate(AllMainBicycles, bicycles[index]); 
        }

        html = html.replace(/<%AllMainBicycles%>/g, allMainBicycles);
        ...
        const image = await fs.readFile(`./public/img/${req.url.slice(1)}`);
        ...
        const svg = await fs.readFile(`./public/img/icons.svg`);
...
function replaceTemplate(html,bicycle){
    ...
    if(bicycle.hasDiscount){
        html = html.replace(/<%DISCOUNTRATES%>/g, `<div class="discount__rate"><p>${bicycle.discount}% Off</p></div>`);
    } else {
        html = html.replace(/<%DISCOUNTRATES%>/g, '');
    }
